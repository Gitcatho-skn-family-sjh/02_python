...
pip_commands = [
    "python3 -m venv .venv", # venv라는 이름의 가상환경 생성(가상환경 폴더명 .venv)
    ".venv\\Scripts\\activate",  # windows
    "source .venv/bin/activate",  # mac, lnux
    "python -m pip --version",
    "python -m pip install requests", # requests 패키지 설치
    "python -m pip show requests",
    #현재 가상환경에 설치된 패키지 목록을 저장
    "python -m pip freeze > requirements.txt",

    #requirements.txt에 있는 파일을 한번에 설치
...
def find_missing_packages() -> list[str]:
    ...
    missing_packages = [] # 설치 안된 패키지를 저장한 list
